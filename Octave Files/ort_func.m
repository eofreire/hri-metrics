function ort_ref = ort_func(referent, npart)
  % This function calculates a metric (ort_ref) for each unique gesture
  ...
  % Initialize the ort_ref vector with zeros, one entry for each unique gesture
  ort_ref = zeros(num_gestures, 1);
  ...
  for i = 1:num_gestures
    ...
    for j = 1:npart  % number of participants
      ...
      if (~isempty(aux))
        ...
        % Calculate a contribution to the ort_ref value based on the gesture's position
        ...
        % Add this contribution to the total ort_ref value for the current unique gesture
        ort_ref(i) = ort_ref(i) + aux2;
      endif
    endfor
  endfor
endfunction
